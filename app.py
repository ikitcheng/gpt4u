...
def main():
    st.title("GPT-4-U")
    cols = st.columns(2)
    openai_api_key = cols[0].text_input(
    ...
    uploaded_files = cols[0].file_uploader(
    ...
    if len(uploaded_files) > 0:
        for file in uploaded_files:
            ...
            if ".json" not in file.name and not os.path.isfile(
                f"/app/context_data/{file.name}"
            ):
                save_uploadedfile(file, outpath="/app/context_data/")
            elif ".json" in file.name and not os.path.isfile("/app/index.json"):
